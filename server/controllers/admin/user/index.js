...
async function getByBirthdays() {
    try {
        const birthdayUsers = await database`
            select id, "firstName", "lastName", "dateOfBirth", image, membership, level
            from users
            where "dateOfBirth" IS NOT NULL and date_part('day', "dateOfBirth") = date_part('day', CURRENT_DATE + INTERVAL '1day') and date_part('month', "dateOfBirth") = date_part('month', CURRENT_DATE + INTERVAL '1day')`
